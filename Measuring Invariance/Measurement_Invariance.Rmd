...
@landers2015inconvenient encourage researchers to avoid broad classifications such as good and bad regarding samples and sampling methodologies, concluding that online panels may be qualitatively "different" from traditional convenience samples, but they should not be stigmatized simply because they're different.  @cheung2017amazon conducted a thorough review of one of the earliest platforms to emerge within the realm of organziational research, MTurk, and highlighted a number of noted limitations, but provided redcommendations for addressing each limitation while again cautioning against negative stereotypes. @walter2019tale performed a meta-analysis focused on internal consistency estimates as well as IV-DV effects, noting similar results stemming from online panels (when compared to more traditional data sources). 
...
use$flag[use$item61 == 5 & use$item145 == 5 & use$item248 == 2 & use$item308 == 3 & use$careless_long < 20 & use$missing < 200] <- "Good"
...
pilot <- analyses
pilot$from[pilot$from == "ProlificDrop"] <- "Prolific"      ## first just 3 samples - 10/10/23
pilot$from[pilot$from == "ProlificKeep"] <- "Prolific"      ## first just 3 samples - 10/10/23
pilot$from[pilot$from == "QualtricsDrop"] <- "Qualtrics"      ## first just 3 samples - 10/10/23
pilot$from[pilot$from == "QualtricsKeep"] <- "Qualtrics"      ## first just 3 samples - 10/10/23

configural.a <- cfa(model=attiduinal, data=pilot, group="from") # configural invariance
weak.a <- cfa(model=attiduinal, data=pilot, group="from", group.equal="loadings") # weak invariance
strong.a <- cfa(model=attiduinal, data=pilot, group="from", group.equal=c("intercepts", "loadings")) # strong invariance
strict.a <- cfa(model=attiduinal, data=pilot, group="from", group.equal=c("loadings", "intercepts", "residuals"))
# summary(configural.a, fit.measures=TRUE)
...
configural.s <- cfa(model=substantive, data=pilot, group="from") # configural invariance
weak.s <- cfa(model=substantive, data=pilot, group="from", group.equal="loadings") # weak invariance
strong.s <- cfa(model=substantive, data=pilot, group="from", group.equal=c("intercepts", "loadings")) # strong invariance
strict.s <- cfa(model=substantive, data=pilot, group="from", group.equal=c("loadings", "intercepts", "residuals"))


fitsub <- lavTestLRT(configural.s,weak.s,strong.s, strict.s)
write.csv(fitsub, "groupindicessub.csv")

papaja::apa_table(fitsub, # apa contains the data.frame needed for apa_table
...
keep <- rbind(qualtrics.engage.keep, prolificgood, snowball)[,-c(3,8)]

configural.subscreen <- cfa(model=substantive, data=keep, group="from") # configural invariance
weak.subscreen <- cfa(model=substantive, data=keep, group="from", group.equal="loadings") # weak invariance
strong.subscreen <- cfa(model=substantive, data=keep, group="from", group.equal=c("intercepts", "loadings")) # strong invariance
strict.subscreen <- cfa(model=substantive, data=keep, group="from", group.equal=c("loadings", "intercepts", "residuals"))


fitsubscreen <- lavTestLRT(configural.subscreen,weak.subscreen,strong.subscreen, strict.subscreen)
write.csv(fitsubscreen, "groupindicessubscreen.csv")

papaja::apa_table(fitsubscreen, # apa contains the data.frame needed for apa_table
                  caption = "Measurement invariance summary statistics (substantive structure screened responses).",
                  note = "* p < 0.05; ** p < 0.01; *** p < 0.001",
                  landscape = FALSE,
                  escape = F)


configural.attscreen <- cfa(model=attiduinal, data=keep, group="from") # configural invariance
weak.attscreen <- cfa(model=attiduinal, data=keep, group="from", group.equal="loadings") # weak invariance
strong.attscreen <- cfa(model=attiduinal, data=keep, group="from", group.equal=c("intercepts", "loadings")) # strong invariance
strict.attscreen <- cfa(model=attiduinal, data=keep, group="from", group.equal=c("loadings", "intercepts", "residuals"))


fitattscreen <- lavTestLRT(configural.attscreen,weak.attscreen,strong.attscreen, strict.attscreen)
write.csv(fitattscreen, "groupindicesattscreen.csv")

papaja::apa_table(fitattscreen, # apa contains the data.frame needed for apa_table
                  caption = "Measurement invariance summary statistics (substantive structure screened responses).",
                  note = "* p < 0.05; ** p < 0.01; *** p < 0.001",
                  landscape = FALSE,
                  escape = F)
...
#scales %>%
#  group_by(from) %>%
#  summarize(cor=cor(absorb,vigor,dedicate, use="complete.obs"))
